...
struct HandTracker
{
	...
	float  segment_scale       = 0.17f; //tamaño de la mano por defecto de 17 cm
	...
	HandTracker(bool _useLeft = false) : useLeft(_useLeft), handmodel(LoadHandModel(useLeft)), othermodel(LoadHandModel(useLeft)), // two models as one might be used in 2nd thread
		cnn(PoseInitializerCNN("../assets/handposeddPruebaPalmayPuñoTodosDatasets.cnnb")),          //Aqui la CNN queramos usar, ahora esta la mia 																		
		//cnn(PoseInitializerCNN("../assets/handposedd.cnnb")),   //may want to train cnn for specific camera and use case, esta es la cnn por defecto original
